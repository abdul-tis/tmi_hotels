...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		//print $this->db->last_query();die('Ko');
	...
	public function checkPasswordIdValid($userId =0,$password=NULL){
		if(!empty($password)){
			$password = $this->hash_password_db($userId, $password);
			if ($password === TRUE){
				return true;
			}else{
				return false;
	...
	**/
	...
	public function userlogin($identity, $password, $remember=FALSE)
	{ 
		$this->trigger_events('pre_login');
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', email, u.id, password, active,last_login,first_name,last_name,company,phone,image')
